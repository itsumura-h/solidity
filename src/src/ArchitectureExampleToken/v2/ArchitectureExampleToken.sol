...
contract ArchitectureExampleToken is UUPSUpgradeable {
  ...
  function initializeV2(string memory _name, string memory _symbol) public onlyAdmin reinitializer(2) {
    __UUPSUpgradeable_init_unchained();
    // ========== erc20 ==========
    erc20TokenAddress = address(new ERC20Token());
    (bool success,) =
      erc20TokenAddress.delegatecall(abi.encodeWithSelector(ERC20Token.initializeV2.selector, _name, _symbol));
    require(success, "ERC20Token: initialize call failed");
    // ========== role ==========
    roleAddress = address(new Role());
    (success,) = roleAddress.delegatecall(abi.encodeWithSelector(Role.initializeV2.selector));
